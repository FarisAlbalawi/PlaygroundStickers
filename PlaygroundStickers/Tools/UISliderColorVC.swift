...
//  PlaygroundStickers
//
//  Created by Faris Albalawi on 8/15/19.
...
protocol UISliderColorDelegate {
...
class UISliderColorVC: UIViewController, UIGestureRecognizerDelegate {
    
    var SliderColorDelegate : UISliderColorDelegate?
    ...
    func setLastColor(color: UIColor) {
        
        self.lastColor = color
        UIView.animate(withDuration: 0.8, animations: {
            self.redSlider.setValue(Float(color.redValue) * 255, animated:true)
            
            self.greenSlider.setValue(Float(color.greenValue) * 255, animated:true)
            
            self.blueSlider.setValue(Float(color.blueValue) * 255, animated:true)
        })
    ...
    private func darkStyle() {
        ...
        redSlider.minimumTrackTintColor = topColor
        redSlider.maximumTrackTintColor = topColor
        
        greenSlider.minimumTrackTintColor = topColor
        greenSlider.maximumTrackTintColor = topColor
        
        blueSlider.minimumTrackTintColor = topColor
        blueSlider.maximumTrackTintColor = topColor
    ...
    @objc func didPressDone() {
        ...
        if NewColor != nil {
              self.SliderColorDelegate?.sliderColorDone(color: NewColor!)
        }
    ...
    @objc func didPressCancel() {
        removeBottomSheetView()
        if lastColor != nil {
            self.SliderColorDelegate?.sliderColorCancel(color: lastColor!)
        }
